(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const createHighShelvingFilter = (fd, Q, g) => {
            const feedforwards = [];
            const feedbacks    = [];
            ...
            feedforwards[0] = ((1 + g) + (Math.sqrt(1 + g) * (2 * Math.PI * fc) / Q) + (4 * Math.pow(Math.PI, 2) * Math.pow(fc, 2))) / d;
            feedforwards[1] = ((8 * Math.pow(Math.PI, 2) * Math.pow(fc, 2)) - (2 * (1 + g))) / d;
            feedforwards[2] = ((1 + g) - (Math.sqrt(1 + g) * (2 * Math.PI * fc) / Q) + (4 * Math.pow(Math.PI, 2) * Math.pow(fc, 2))) / d;
